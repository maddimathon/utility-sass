...
const colourValuesParsedEntries = colourValueEntries.map(([space, value]) => {
    switch (space) {
        ...
        case 'hex':
            return [
                space,
                { space, r: 0xFF, g: 0x88, b: 0xCC },
            ];
    }
});
export const colourValuesParsed = Object.fromEntries(colourValuesParsedEntries);
function _functionArrFilter(test, arr) {
    // @ts-expect-error
    const func = test.replace(/a$/i, '');
    return arrayUnique([
        ...arr,
        ...arr.map((str) => str.replace(/\s*([\(|\)|,|\/])\s*/g, '$1')),
    ]);
}
